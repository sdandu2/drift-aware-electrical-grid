...
def build_extended_model_registry() -> dict:
    models = {
        "extra_trees_v0_1": ExtraTreesRegressor(
            n_estimators=300,
            max_depth=22,
            min_samples_leaf=2,
        ...
        "hist_gradient_boosting_v0_1": HistGradientBoostingRegressor(
            max_iter=350,
            learning_rate=0.045,
            max_leaf_nodes=45,
        ...
        "mlp_neural_network_v0_1": Pipeline(
            steps=[
                ("scaler", StandardScaler()),
                (
                    "model",
...
def should_use_training_sample(model_version: str) -> bool:
    slow_models = {
        "svr_rbf_v0_1",
    }

    return model_version in slow_models
...
def fit_and_evaluate_extended_models(
    models: dict,
    x_train: pd.DataFrame,
    y_train: pd.Series,
    ...
    for model_version, model in models.items():
        ...
        if should_use_training_sample(model_version):
            x_fit, y_fit = sample_training_data(
                x_train=x_train,
                y_train=y_train,
                max_rows=MAX_SVR_TRAIN_ROWS,
            )
            print(f"{model_version} using sampled training rows: {len(x_fit)}")
        else:
            x_fit = x_train
            y_fit = y_train
        ...
        validation_metrics["training_rows_used"] = len(x_fit)
        test_metrics["training_rows_used"] = len(x_fit)
...
def save_models(
    ...
    best_model_path = CHALLENGER_MODEL_DIR / f"best_extended_challenger_{region_key}.joblib"
    joblib.dump(trained_models[best_model_version], best_model_path)
    output_paths["best_extended_challenger"] = str(best_model_path)
...
def save_metrics_and_metadata(
    ...
    metrics_path = EVALUATION_DIR / f"extended_ml_metrics_{region_key}.csv"
    leaderboard_path = TABLES_DIR / f"extended_ml_model_leaderboard_{region_key}.csv"
    metadata_path = MODEL_METADATA_DIR / f"extended_ml_training_metadata_{region_key}.json"
    ...
    metadata = {
        ...
        "notes": {
            "svr_rbf_v0_1": f"RBF SVR uses a capped training sample of {MAX_SVR_TRAIN_ROWS} rows for runtime control."
        },
    }
...
def print_training_summary(
    ...
    print("")
    print("Extended ML Training Complete")
...
def main() -> None:
    ...
    models = build_extended_model_registry()

    metrics_df, trained_models = fit_and_evaluate_extended_models(
        models=models,
        x_train=x_train,
        y_train=y_train,
